...
with tabs[0]:
        ...
        with cols[idx % 3]:
            ...
            if data:
                ...
                with st.container(border=True):
                    ...
                    # ADX Proxy Calc (Efficiency Ratio)
                    change = df_calc[col_p].diff()
                    volatility_adx = change.abs().rolling(window=14).sum()
                    direction_adx = change.rolling(window=14).sum().abs()
                    er = direction_adx / volatility_adx
                    adx_val = (er * 100).fillna(0).iloc[-1]
                    ...
                    # Weekly Trend from signal
                    w_trend = "N/A"
                    w_color = "gray"
                    if signals:
                        last_sig_obj = signals[0]
                        if 'weekly_trend' in last_sig_obj and last_sig_obj['weekly_trend']:
                            w_trend = last_sig_obj['weekly_trend']
                            w_color = "green" if w_trend == "BULLISH" else "red" if w_trend == "BEARISH" else "gray"
                    ...
                    s_col2.markdown(f"RSI: {last_rsi:.0f} | ADX: {adx_val:.0f} (<span style='color:{trend_color}'>{trend_strength}</span>)", unsafe_allow_html=True)
                    s_col2.markdown(f"Weekly: <span style='color:{w_color}'>{w_trend}</span>", unsafe_allow_html=True)
    ...
    with st.expander("📚 How to read these charts? (Legend & Education)"):
        st.markdown("""
        ### 1. Price Chart (Top Panel)
        *   **Blue Line:** The actual price of the asset.
        *   **Orange Dashed Line (SMA 50):** Average price over the last 50 days.
            *   Price **above** SMA 50 = 📈 **Uptrend** (Healthy).
            *   Price **below** SMA 50 = 📉 **Downtrend** (Weak).
        *   **Grey Shaded Area (Bollinger Bands):** Standard deviation channel.
            *   Price hitting the **Upper Band** = Potentially expensive (Overbought).
            *   Price hitting the **Lower Band** = Potentially cheap (Oversold).
        *   **Triangles:** Actual signals generated by the bot (🟢 BUY, 🔴 SELL).

        ### 2. MACD (Middle Panel)
        *   **Green Bars:** Momentum is shifting up (Buyers are stronger).
        *   **Red Bars:** Momentum is shifting down (Sellers are stronger).
        *   **Strategy:** We look for the histogram to flip from Red to Green (Potential Buy) or Green to Red (Potential Sell).

        ### 3. RSI (Bottom Panel)
        *   **Relative Strength Index (0-100).**
        *   **Above 70 (Red Line):** 🔥 **Overbought**. Price has risen too fast, high risk of correction. Ideally, wait or sell.
        *   **Below 30 (Green Line):** 🧊 **Oversold**. Price has fallen too fast, potential bounce. Good buying opportunity.

        ### 4. Stats & Indicators
        *   **ADX (Average Directional Index):** Measures trend strength.
            *   **> 25 (Trend):** Market is moving strongly in one direction. Trust MACD.
            *   **< 25 (Range):** Market is moving sideways. Trust RSI and Bollinger Bands.
        *   **Weekly Trend:** The "Big Picture" trend. Never bet against it unless you are catching a quick bounce.
        """)
